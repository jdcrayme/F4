// f4-world-viewer/src/fair_mutex.hpp
//
// FairMutex — a first-in-first-out (ticket-order) mutex.
//
// CAMP-HOST-3 provenance: RELOCATED from f4-simulation (fair_mutex.hpp)
// alongside the runner it exists to serve. Pacing is a host concern
// (Docs/CAMP_HOST_PLAN.md §2.2) and a FIFO mutex is sync machinery, not
// engine state — the viewer owns both now; the engine ships neither.
//
// WHY IT EXISTS (the "campaign time doesn't advance" regression):
// the viewer's frame scope holds the campaign session lock for
...
// waiter essentially every time (measured on a 2-core box: the runner's
// worker advanced 0.0 sim-seconds over 3 wall seconds — a COMPLETE
// starvation, campaign time frozen while the UI stayed perfectly
// smooth).
//
...
//
// Dependencies: std only, header-only, C++20.
#pragma once
