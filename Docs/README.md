...
| Campaign employment (the campaign→sim kill chain) | `CAMP_EMPLOYMENT_PLAN.md` | EMPL-1 LANDED (A-G release chain), EMPL-1a (impact precision), EMPL-1b (recorder intended-path fields), EMPL-2 (campaign-path AAR — the receiver joins, latches, and refuels from campaign-spawned flights; the fleet-scale on-save demo is the named follow-up). EMPL-3 LANDED via QC-ANCHOR. |
