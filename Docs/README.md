...
| Mission QC (the sanity-check layer) | `MISSION_QC_COOKBOOK.md` | As-built workflow: the three QC layers (gates → ledgers → viewer), per-mission-type filtered `campaign_qc` runs, and `scripts/qc_missions.py` (the per-category matrix runner with expectations). §5 documents the strike employment gap the first sweep caught (armed, no target, no release — saved and ladder-generated alike). §7's showcase scenarios are the open tranche. |
