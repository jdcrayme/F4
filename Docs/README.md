...
| Campaign host (the engine contract) | `CAMP_HOST_PLAN.md` | Draft v1 — HOST-1/2/3 (contract, event stream + journal, the viewer as client) and CMD-1/2 (the full v1 command surface: RoE, retask, abort, priority, the tick-exact replay) shipped; CAMP-ATM/INIT/SCALE/DOM pending. |
