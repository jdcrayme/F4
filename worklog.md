...
---
Task ID: SYMBOL-TWOTONE-1
Agent: main (Super Z)
Task: Two-tone team colors + composite unit icons on IcoWork (re-land;
the previous session's sandbox reset lost the unpushed commit and its
patch file). Answers the user's two design questions in code:
(1) YES — SVG placeholder paints are runtime-substituted per owning
team (black & white become e.g. dark red & red); (2) YES — the
frame/glyph split is retired in favor of one composite SVG per unit
type (no glyph is reused across frames).

Work Log:
- svg_import: white ("#ffffff"/"white"/"#fff") now maps to the Fill
  role (black stays Outline). Icons are authored black-on-white (the
  MIL-STD-2525 convention) and BOTH placeholder paints are replaced at
  draw time: white -> team PRIMARY (background/frame fill), black ->
  team SECONDARY (glyph/contrast strokes). data-color-role still
  overrides. No corpus SVG used white, so only the contract changed.
- symbol_library.hpp: documented the two-tone semantics of the existing
  (fill, outline) draw pair — fill_col = team primary, outline_col =
  team secondary. No API break; the roles were already runtime colors.
- entity_render: frame_key_for_unit_class() + glyph_key_for_unit()
  replaced by unit_symbol_key_for(cls, subtype) returning ONE composite
  key per unit type ("unit_armor", "unit_fighter", ...). EntityIconInfo
  collapsed to {symbol_key, valid}; RenderEntityIcon draws a single
  symbol (the composite SVG carries frame + glyph). Frame files stay
  live: Flight/Package icons ARE their bare frames, and mapped classes
  with unknown subtypes fall back to the bare frame.
- viewer_state.hpp: TeamPalette {primary, secondary} +
  team_palette_for_owner() — hand-tuned per-team pairs (enemy red/dark
  red, friendly blue/navy, ROK green/forest, Japan yellow/olive, DPRK
  orange/rust, PRC magenta/plum, neutral gray) instead of the old
  fill x 0.4 derived outline (which turned yellow into mud), plus
  dimmed_palette() for the team/mission filter fade. color_for_owner()
  stays the single-color source (destination lines, waypoint dots,
  selection rings, legend).
- canvas.cpp: objective + unit + live-aircraft icon draws switched to
  the palette (primary/secondary). The live-aircraft fighter marker
  keeps glyph_fighter.svg (the per-type rotating disaggregated icons
  are the next tranche and ride on that call site); grounded wash
  toward gray now applies to both palette entries.
- symbols/README.md: documents the two-tone painting contract and the
  four corpus groups (obj_* live, unit_* live composites, frame_* live
  fallbacks, glyph_* authoring sources + the future deagg type-icon
  vocabulary).
- Tests: test_symbol_mapping rewritten for composite keys (8 tests);
  test_entity_render EntityIconInfo expectations updated (32 pass);
  test_svg_import gained TwoTonePlaceholders_WhitePrimaryBlackSecondary
  and EveryMappedKeyHasAnSvg now walks unit_symbol_key_for (16 pass);
  test_symbol_library 10 pass.
- NOTE (pipeline bug found, not fixed here): svg_import silently maps
  white on currentColor-corpus files only through explicit fill attrs;
  the round-trip exporter writes Fill role as currentColor, so white
  never round-trips as white — role-stable, fine for the contract.

Stage Summary (SYMBOL-TWOTONE-1 — two-tone + composites LANDED):
- Every icon color on the map is now team-dependent: the SVG's
  black-and-white placeholders get replaced at runtime by the owning
  team's two-tone palette, per draw call. Teams read as color families
  (verified in an end-to-end viewer screenshot: blue/navy vs
  red/dark-red vs magenta/plum front line).
- One icon per unit type: 19 composite unit_* SVGs + 6 frame fallbacks
  resolve every (class, subtype) the decoder can produce; the
  frame/glyph draw-order split is gone from the render path.
- Full suite green under Xvfb: 2313/2313 ctest (renderer suites:
  symbol_mapping 8, entity_render 32, symbol_library 10, svg_import
  16). symbol-grid renders all 75 icons; composites show frame+glyph
  merged correctly.
- Patch file for user testing: /home/z/my-project/download/
  0001-Two-tone-team-colors-composite-unit-icons.patch (git
  format-patch vs 2a5011e).
